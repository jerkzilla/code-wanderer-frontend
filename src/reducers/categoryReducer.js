export default function categoryReducer(
  state = { categories: [], resources: [], wanders: [] },
  action
) {
  switch (action.type) {
    ...
    case "ADD_RESOURCE":
      let categories = state.categories.map((category) => {
        if (category.id === action.payload.category_id) {
          return {
            ...category,
            resources: [...category.resources, action.payload],
          };
        } else {
          return category;
        }
    ...
    case "DELETE_RESOURCE":
      ...
      let resourcesWithoutDeletedResource = categoryWithResourceToDelete.resources.filter(
        (resource) => {
